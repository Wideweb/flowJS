...
import CastAnimation from './cast.animation';
...
export default class AnimationManager {
    ...
    load(container: Container): void {
        const move = new MoveAnimation(this.resource);
        move.load(container);

        const stop = new StopAnimation(this.resource);
        stop.load(container);

        const attack = new AttackAnimation(this.resource);
        attack.load(container);

        const cast = new CastAnimation(this.resource);
        cast.load(container);
